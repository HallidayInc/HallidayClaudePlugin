...
5. For API endpoint details, Claude uses `Grep` on `${CLAUDE_PLUGIN_ROOT}/sources/api/openapi.json` for the relevant endpoint, then `Read` only the matching lines (±50 lines of context)
...
- Should NOT Read `${CLAUDE_PLUGIN_ROOT}/sources/api/openapi.json` or docs files whole — use Grep to find relevant sections
