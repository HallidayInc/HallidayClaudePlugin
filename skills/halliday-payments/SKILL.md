...
**CRITICAL: Never Read these files whole (except sdk-types.d.ts). They are too large and will pollute your context. Use Grep to find relevant sections, then Read only those lines.**

| Source file | ~Tokens | How to use |
|-------------|---------|------------|
| `sources/sdk-types.d.ts` | ~6K | **Safe to Read whole.** Contains all TypeScript types, `openHallidayPayments()` params, widget config options, wallet interface. Load this when verifying parameter names or types. |
| `sources/openapi.json` | ~47K | **Grep only.** Use `Grep` to search for endpoint paths (e.g. `/payments`), schema names (e.g. `QuoteRequest`), or field names. Then `Read` only the matching lines ±50 lines of context. |
| `sources/docs-full.txt` | ~49K | **Grep only.** Use `Grep` to search for topic keywords (e.g. "onramp", "cross-chain", "EIP-712"). Then `Read` only the matching section (typically 50-150 lines). |
...
**Never load all three source files. Never load openapi.json or docs-full.txt whole.**
